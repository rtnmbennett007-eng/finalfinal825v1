...
// Dedicated Google Drive Health Endpoint (separates Vercel API health from Google Drive status)
app.get(['/api/health/drive', '/health/drive'], async (req, res) => {
  res.setHeader('Content-Type', 'application/json');
  try {
    ...
    res.status(200).json({
      ...
      error: err?.message || 'Google Drive diagnostic evaluation failed',
      timestamp: new Date().toISOString(),
      version: '1.0.0',
    });
  }
