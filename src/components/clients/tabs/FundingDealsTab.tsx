...
export const FundingDealsTab: React.FC<FundingDealsTabProps> = ({
  ...
  const [dealForm, setDealForm] = useState<Partial<FundingDeal>>({
    ...
    status: 'APPROVED',
    assignedStaff: client.assignedStaff || currentUser?.name || 'Dana',
  ...
  const handleOpenAddDeal = (preset?: Partial<FundingDeal>) => {
    ...
    setDealForm({
      ...
      fundingAmount: preset?.fundingAmount || 50000,
      fee: preset?.fee !== undefined ? preset.fee : 1495,
      percentage: preset?.percentage !== undefined ? preset.percentage : 6.9,
      termLength: preset?.termLength || '24 Months',
      status: preset?.status || 'APPROVED',
      assignedStaff: client.assignedStaff || currentUser?.name || 'Dana',
      lenderStatus: preset?.lenderStatus || 'APPROVED',
      lenderName: preset?.lenderName || 'Maple Direct Capital',
      lenderContact: preset?.lenderContact || 'underwriting@mapledirect.com',
