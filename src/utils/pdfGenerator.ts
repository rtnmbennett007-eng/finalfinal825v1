...
export function generateUnderwritingReportPdf(
  ...
  autoTable(doc, {
    ...
    body: [
      ['Monthly Verified Business Revenue', client.monthlyRevenue ? `$${Number(client.monthlyRevenue).toLocaleString()}` : (underwriting.debtService?.monthlyBusinessRevenue ? `$${Number(underwriting.debtService.monthlyBusinessRevenue).toLocaleString()}` : 'Not Provided')],
      ['Average Monthly Bank Deposits', underwriting.debtService?.monthlyDeposits || underwriting.avgMonthlyDeposits ? `$${Number(underwriting.debtService?.monthlyDeposits || underwriting.avgMonthlyDeposits).toLocaleString()}` : (client.monthlyRevenue ? `$${Number(client.monthlyRevenue).toLocaleString()}` : 'Not Provided')],
      ['Existing Monthly Obligations', underwriting.debtService?.existingMonthlyObligations ? `$${Number(underwriting.debtService.existingMonthlyObligations).toLocaleString()}` : 'None Disclosed'],
      ['Proposed New Debt Payment', underwriting.debtService?.proposedNewPayment ? `$${Number(underwriting.debtService.proposedNewPayment).toLocaleString()}` : '$0'],
      ['Total Projected Monthly Obligations', underwriting.debtService?.estimatedTotalObligations ? `$${Number(underwriting.debtService.estimatedTotalObligations).toLocaleString()}` : '$0'],
      ['Debt-Service Coverage Ratio (DSCR)', underwriting.debtService?.estimatedDebtServiceRatio ? `${underwriting.debtService.estimatedDebtServiceRatio}x` : 'N/A'],
      ['Payment-to-Revenue Ratio (P/R %)', underwriting.debtService?.estimatedPaymentToRevenueRatio ? `${underwriting.debtService.estimatedPaymentToRevenueRatio}%` : 'N/A'],
      ['Obligation Analysis Commentary', underwriting.debtService?.obligationNotes || 'Evaluation of verified cashflow vs existing obligations.'],
    ],
    theme: 'grid',
    headStyles: { fillColor: NAVY, textColor: [255, 255, 255], fontStyle: 'bold', fontSize: 8.5 },
    bodyStyles: { fontSize: 8, textColor: DARK_SLATE },
